...
/**
 * @ClassName: MouseImpl
 * @Description: 鼠标增删改查实现类
 * @author: 汪哒哒
 * @date: 2019年7月10日 上午10:37:49
 */
public class Mouse_Impl implements MouseDao {
...
//这是使用封装思想实现高级查询的方法
	...
	@Override
	public List page_advanced_Query(PageResult rs, ProductQueryObject go) {
		//System.out.println(rs.toString());
		String querySQL = "select * from mouse"+go.getQuerySQL()+" LIMIT ?,?";// 使用基于基类扩展而来的函数模板,获取后半部分语句
		go.getParameters().add((rs.getCurrentPage()-1)*rs.getPageSize());
		go.getParameters().add(rs.getPageSize());
		//System.out.println("参数=" + go.getParameters());
		//System.out.println("SQL语句:" + querySQL);
		// System.out.println(sql.hashCode());
		list= JdbcTemplate.Query(querySQL.toString(), new GeneralResultList<>(Mouse.class), go.getParameters().toArray());
