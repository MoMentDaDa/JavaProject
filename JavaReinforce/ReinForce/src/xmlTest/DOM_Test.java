...
//删除E3节点所有元素
	...
	public void test5() throws Exception {
		...
		Element root = dou.getDocumentElement();
		// 通过调用父方法删除节点
		// root.removeChild(root.getElementsByTagName("linkman").item(2));
		// 自己调用父类方法删除节点
		Element E3 = (Element) root.getElementsByTagName("linkman").item(2);
...
//创建xml文件
	...
	public void test6() throws Exception {
		// 使用相对路径获取文件路径
		File f = new File("C:\\Users\\汪哒哒\\Documents\\JavaProject\\JavaReinforce\\ReinForce\\resources\\Students2.xml");
		// 获取Document对象
		DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();
		DocumentBuilder d = factory.newDocumentBuilder();
		Document dou = null;
		if (f.exists()) {
			...
			dou = d.newDocument();
			dou.appendChild(dou.createElement("contacts"));
		}
		Element root = dou.getDocumentElement();
		// 创建Element各个元素
		Element linkmanE3 = dou.createElement("linkman");
